...
func TestReadSkillFrontmatterMetadataObject(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	skillDir := filepath.Join(root, "github-cli")
	if err := os.MkdirAll(skillDir, 0o755); err != nil {
		t.Fatalf("mkdir skill dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
...
Body`), 0o644); err != nil {
	...
	openclaw, ok := fm.Metadata["openclaw"].(map[string]any)
	if !ok {
		t.Fatalf("expected openclaw metadata object, got: %T", fm.Metadata["openclaw"])
	}
	if openclaw["emoji"] != "🐙" {
		t.Fatalf("unexpected emoji: %v", openclaw["emoji"])
	}
	requires, ok := openclaw["requires"].(map[string]any)
	if !ok {
		t.Fatalf("expected requires object, got: %T", openclaw["requires"])
	}
	bins, ok := requires["bins"].([]any)
	if !ok || len(bins) != 1 || bins[0] != "gh" {
		t.Fatalf("unexpected bins: %#v", requires["bins"])
	}
}
