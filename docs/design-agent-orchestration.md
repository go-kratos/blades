...
- 子 Agent 通过 `tools.ActionLoopExit` 在 `event.TurnEnd` 上挂载退出信号；`true` 表示 escalated（以 `blades.ErrLoopEscalated` 报错终止），`false` 表示正常退出
...
2. 先运行 root agent，监听其输出流中 `event.TurnEnd` 携带的 `Actions[ActionHandoffToAgent]`；命中后停止 root，把同一份输入 channel（或基于路由结果重新构造的 prompt）转交给目标子 Agent
