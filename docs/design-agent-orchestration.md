...
- 子 Agent 在输出流上发出 `event.LoopExit{Escalate}` 帧（由 `tools.ErrLoopExit` sentinel 翻译而来）：`Escalate=true` 表示向外层升级（以 `blades.ErrLoopEscalated` 报错终止），`Escalate=false` 表示正常退出
...
`LoopState` 暴露 `Iteration / FirstInput / LastOutput`，全部建立在 `event.Input` / `event.Output` 之上。Loop 不直接读取模型上下文；需要更复杂判断时通过 Agent 中间件、`tools.ExitTool` 或 session state 实现，避免把模型推理塞进 flow 层。

## Routing

`RoutingAgent` 用于"由 LLM 决策接下来交给哪个子 Agent"的单跳分发：

1. 使用 `internal/handoff` 构造 root agent 的指令与 `handoff` 工具
2. 先运行 root agent，监听其输出流中的 `event.Handoff{Agent}` 帧（由 `tools.ErrHandoff` sentinel 翻译而来）；命中后停止 root，把同一份输入 channel（或基于路由结果与 `event.Handoff.Carry` 重新构造的 prompt）转交给目标子 Agent
3. 目标未命中时回退最近一条非空文本输出；都没有则返回错误
...
DeepAgent 仍然是一个普通 `blades.Agent`，可以再被 Sequential/Parallel/Loop/Routing 组合。任务分解的具体协议（todos 字段、task 工具的入参）属于 `internal/deep` 实现细节，flow 层不暴露。
