...
不新增 `settings/`、`app/`、`host/`、`channel/` 或 `workspace/` 核心包。配置文件、环境变量、默认值合并、transport 接入、run lifecycle、workspace 边界都属于应用装配职责，放在具体应用的 `internal/` 包中，例如 `cmd/blades/internal/app`、`cmd/blades/internal/channel` 和 `cmd/blades/internal/workspace`。
...
7. 用 `cmd/blades/internal/*` 和 examples 展示应用层如何接入 channel、workspace、配置、run lifecycle 和主动通知。
