...
// It is split from Provider because counting is often backed by a separate
// endpoint or local encoder.
...
- `TokenCounter.CountTokens` 返回 `TokenCount`。`Input` 是完整 request 输入估算，包含结构化输出 Schema 等请求选项；当实现能拆分 system/messages/tools 三段时填充分段字段，`HasSegments()` 返回 true。
