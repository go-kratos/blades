...
核心只提供 `Hook` 接口、`HookFunc` 适配器、6 个 `On*` helper 与 `Chain` 顺序组合 helper。根包 `WithHooks(...hook.Hook)` 直接接收 Hook 列表，按注册顺序串行派发。动态启停、按应用配置分发、按事件类型路由、异步 fan-out、热更新等聚合行为均由应用层自行实现一个 `hook.Hook` 并通过 `WithHooks` 注入即可，不进入核心 surface area。
...
- r22：单一 `Hook` 接口（`Handle(ctx, Event) error`）、sealed `hook.Event`、六类核心 event、统一签名的泛型 helper、Hook 内部 type switch、Mutator 细粒度 setter 与 abort 语义矩阵均已覆盖。
