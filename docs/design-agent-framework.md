...
│    └── ProviderAdapter     blades.Message ↔ Provider Message     │
├─────────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────────┤
│  Provider 层                                                     │
│    ├── contrib/anthropic   Claude（含 Prompt Cache + Adapter）   │
│    ├── contrib/openai      GPT / o-series（含 Adapter）          │
│    ├── contrib/gemini      Gemini（含 Adapter）                  │
...
| 原则 | 说明 | 参考来源 |
|------|------|---------|
| Event 类型安全边界 | 用户侧只接触 InputEvent/OutputEvent，Message 是内部实现细节 | 原创设计 |
...
| 消息边界 | 应用层 Event 与 LLM 层 Message 通过 Internal Service Layer 显式转换 | pi-agent convertToLlm |
| Provider 适配 | ProviderAdapter 处理 blades.Message ↔ Provider Message 双向转换 | 原创设计（显式化现有 contrib 中的隐式转换） |
...
新设计将职责分离到不同层：Event 是用户协议，Message 是 Internal Service Layer 的内部表示，Provider Message 是最底层的 LLM 格式。用户只需要知道"InputEvent 进，OutputEvent 出"。

Service Layer 进一步拆分为两层：
- **Internal Service Layer**（ContextBuilder、ResponseAdapter、SessionRecorder、ProviderAdapter）— Agent Loop 的私有实现，不暴露给用户
...
type ResponseAdapter interface {
    Adapt(stream Generator[*ModelResponse, error]) <-chan OutputEvent
}
```
...
```go
// ProviderAdapter 负责 blades.Message 与 Provider 原生消息格式之间的双向转换。
// 不同 Provider 的消息格式差异在此层统一处理：
// - Anthropic：tool_use/tool_result 拆分为独立消息、cache_control 注入
// - OpenAI：function_call 格式转换、structured output 适配
// - Gemini：FunctionCall/FunctionResponse 格式转换
//
// 当前代码中 contrib/anthropic 已在做这个转换（applyEphemeralCache、tool message 拆分），
// 新设计将这个职责显式化为接口，每个 Provider 实现自己的 Adapter。
type ProviderAdapter interface {
    // ToProvider 将 blades.Message 转换为 Provider 原生请求格式。
    ToProvider(ctx context.Context, req *ModelRequest) (any, error)
    // FromProvider 将 Provider 原生响应转换为 blades.Message。
    FromProvider(ctx context.Context, resp any) (*Message, error)
}
```

注意：`MessageConverter`（应用层 → LLM 层）和 `ProviderAdapter`（LLM 层 ↔ Provider 层）是两个不同的边界。
`MessageConverter` 处理 CustomPart 过滤和转换，`ProviderAdapter` 处理 Provider 特定的消息格式差异。
...
User                                Agent Loop
  ...
  │                                     │     → *ModelRequest
  │                                     ├─→ ProviderAdapter.ToProvider(request)
...
| `ModelProvider` | 保留 | Provider 接口不变，新增 `ProviderAdapter` 处理消息格式转换 |
| `Session` | 保留 | 仍存储 `[]*Message`，通过 `SessionRecorder` 从 OutputEvent 中提取 |
...
```
blades/                         根包：核心接口和类型
├── event.go                    InputEvent/OutputEvent 接口 + 所有 Event 类型 + Once()
├── agent.go                    Agent 接口 + NewAgent()
├── message.go                  Message（内部类型，Internal Service Layer 使用）
├── session.go                  Session 接口（不含实现）
├── model.go                    ModelProvider 接口 + ProviderAdapter 接口
├── errors.go                   公共错误
│
├── session/                    会话持久化
...
│
├── compact/                    上下文压缩（原 compress/，与文档术语一致）
...
│
├── graph/                      DAG 执行器（独立子系统，见"graph 定位"章节）
...
│
├── contrib/                    Provider 实现（每个含 ProviderAdapter）
│   ├── anthropic/              contrib/anthropic.Claude + anthropic.Adapter
│   ├── openai/                 contrib/openai.Chat + openai.Adapter
│   ├── gemini/                 contrib/gemini.Gemini + gemini.Adapter
...
| 变化 | 原因 |
|------|------|
| 根包新增 `event.go` | InputEvent/OutputEvent 是核心类型，所有包都依赖 |
...
```
blades（根包：InputEvent, OutputEvent, Agent, Message, Session, ProviderAdapter 接口）
  ↑
  ├── tools, session, memory, prompt, compact, token, hook, permission, retry
  ↑
  ...
  ├── evaluator（依赖 blades）
  ↑
  ├── contrib/*（依赖 blades, tools；每个实现 ProviderAdapter）
  ↑
  └── cmd/blades（依赖所有）
```

依赖方向严格单向，无循环。根包只定义接口，不依赖任何子包。
注意：`compact` 包不依赖 `blades.Agent`，通过 `Summarizer` 函数注入避免循环依赖。

### 各包的核心导出类型

| 包 | 核心类型 | 使用时读作 |
|---|---------|----------|
| `blades` | `Agent`, `InputEvent`, `OutputEvent`, `Message`, `Session` | `blades.Agent` |
...
2. **双层消息转换边界** — `MessageConverter`（应用层 → LLM 层）处理 CustomPart 过滤和转换；`ProviderAdapter`（LLM 层 ↔ Provider 层）处理 Provider 特定的消息格式差异（Anthropic tool_use/tool_result 拆分、OpenAI function_call 格式等）。两个边界职责分明，Provider 只需实现自己的 Adapter。
...
func (a *agent) callProvider(ctx context.Context, req *ModelRequest) (Generator[*ModelResponse, error], error) {
...
type Counter interface {
    Count(messages ...*blades.Message) int64
}
...
// 4. graph.Handler 中如果需要调用 LLM，通过注入的 ModelProvider 实现
```
...
**contrib/ 包**：Provider 接口不变，新增 `ProviderAdapter` 实现。
- `contrib/anthropic`：将现有 `applyEphemeralCache` 和 tool message 拆分逻辑迁移到 `anthropic.Adapter`
- `contrib/openai`：将 function_call 格式转换迁移到 `openai.Adapter`
...
- [ ] 实现 `ProviderAdapter` 接口和 Anthropic/OpenAI 实现
...
- [ ] 迁移 `contrib/` Provider 实现（新增 ProviderAdapter）
