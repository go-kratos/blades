...
`contextBuilder` 是 Session / Prompt / Compact 与 `*model.Request` 之间的装配逻辑，位于根包私有实现，按有序 pipeline 装配——**先装配完整请求，再判断是否压缩消息**：`snapshot ← session.Messages(ctx)`；`systemParts ← prompt.Builder.Build(ctx)`（memory 召回在此发生）；装配 `System / Messages / Tools / Options` 后按完整 request 计数；超过阈值时再调用 `compactor.Compact(ctx, compact.Request{Messages: snapshot, TokenCounter: counter})`，并将压缩视图写入最终请求。`WithContextBudget` 配置 input/system/messages/tools 预算，`WithTokenCounter` 显式配置完整 request 计数；未配置时使用 `model.ApproxTokenCounter`，且其总量包含结构化输出 Schema 等请求选项，不从 provider 自动探测；根包把 `ContextInfo` 暴露给 prompt/memory 构建期，把 `ContextStats` 暴露给 model hooks/provider 调用期。Compactor 仍仅处理 messages，prompt builder 仅构建 system，二者互不感知。详见 [design-context-management.md](design-context-management.md)。
