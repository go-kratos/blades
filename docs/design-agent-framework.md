...
工具自身通过 `tools.NewContext` / `tools.FromContext` 取得每次调用的 `ToolContext`（`ID` / `Spec`），由 Agent Loop 在调用 `Tool.Handle` 前注入；`ToolContext` 仅暴露调用元数据，不承载控制信号。工具签名仍为 `Handle(ctx context.Context, input json.RawMessage)`。控制流信号通过 sentinel error 返回，由 Agent Loop 翻译为专用 sealed Output 帧 `event.LoopExit` / `event.Handoff`，紧跟在产生它的 `event.ToolEnd` 之后发出。
...
| Runtime 根包内置 | 默认 Agent Loop 是根包 `llmAgent` 的内部机制，不暴露公开 `loop/` 包；loop/step/tool wave 逻辑全部内联在 `agent.go` 中 |
| 无 maxSteps | Agent Loop 不设步数上限，靠 model stop reason（无 tool calls）+ tool 控制信号（LoopExit/Handoff）+ ctx 取消 + Abort 事件终止循环 |
...
```
blades/
├── agent.go                    Agent 接口 + NewAgent + llmAgent 实现（loop/step/tool wave 全部内联）
├── option.go                   AgentOption + WithModel/WithTools/WithPolicy/WithHooks/WithCompact/WithPrompt/WithDescription/WithToolsResolver
├── agent_tool.go               NewAgentTool(Agent) tools.Tool —— 把 Agent 暴露为工具的根包适配器
...
│   ├── control.go              Abort{Reason}, Pause{}, Resume{}（Input）；LoopExit{ToolID, ToolName, Escalate}, Handoff{ToolID, ToolName, Agent, Carry}（Output）
...
│   └── terminal.go             StepEnd{StopReason, Usage}, TurnEnd{Parts, StopReason, Usage, Err}, Error{Err}, Done{}；StopReason/Usage 类型
│
├── model/
...
│   ├── capability.go           ReadOnlyTool / DestructiveTool / StreamingTool（3 个可选能力接口）
│   ├── resolver.go             Resolver（List + Resolve）
│   ├── filter.go               ToolFilter + AllowOnly/Disallow/ReadOnlyFilter/And/Or
...
- `RunLive` 适用于交互式界面、长连接和 steering 场景；input channel 关闭仅表示"无更多输入"，**不**强制终止运行——终止仍由 `event.Abort` 或 `ctx.Done()` 表达。
...
- **根包 `llmAgent`**：*单个* Agent 内部的运行循环（follow-up loop → step loop → tool wave）。无公开 `loop/` 包，逻辑全部内联在 `agent.go`。
...
- **根包 `blades.NewAgentTool`**：把一个 `blades.Agent` 暴露为 `tools.Tool`，让另一个 Agent 通过工具调用启用它；属于"Agent 是顶层 first-class 概念"语义，故落在根包 `agent_tool.go` 而非 `flow/`。签名为 `NewAgentTool(agent Agent) tools.Tool`。
...
| `event` | `Input`（sealed：`input()`）, `Output`（sealed：`output()`）, `Prompt`, `Steer`, `Abort{Reason}`, `Pause`, `Resume`, `TextDelta`, `ThinkingDelta`, `PartStart`, `PartDelta`, `PartEnd`, `ToolStart`, `ToolDelta`, `ToolEnd`, `LoopExit{ToolID,ToolName,Escalate}`, `Handoff{ToolID,ToolName,Agent,Carry}`, `StepEnd`, `TurnEnd`, `Error`, `Done`, `StopReason`, `Usage`；构造糖：`NewPromptText`, `NewSteerText` | `event.NewPromptText("hi")` |
| `model` | `Message{Role, Parts []content.Part}`, `Role`, `RoleUser`/`RoleAssistant`/`RoleTool`, `Provider`(Name+Generate+Stream `iter.Seq2`), `EmbeddingProvider`, `Request{Model, System, Messages, Tools []tools.ToolSpec, Options}`, `Response{Message, StopReason, Usage}`, `Chunk{Parts, StopReason, Usage}`, `Option` sealed（`CacheHint`/`ReasoningEffort`/`ResponseFormat`/`Sampling`）, `Usage`, `StopReason`, `Collect`, `MergeOptions` | `model.Provider` |
| `tools` | `Tool`(Spec+Handle 两方法), `ToolSpec{Name, Description, InputSchema, OutputSchema}`, `Result{Parts []content.Part}`, `ReadOnlyTool`/`DestructiveTool`/`StreamingTool`（3 个可选能力接口）, `Resolver`(List+Resolve), `ToolFilter`, `ToolContext`(ID+Spec), `NewContext`/`FromContext`, `ErrLoopExit`/`ErrHandoff` | `tools.Tool` |
...
- 把 `model.Chunk` 转成 `event.TextDelta`、`event.ToolStart`、`event.StepEnd` 等输出事件。
- 执行 tool wave：policy check → 并发 Handle → `event.ToolEnd` → `content.ToolResult` 追加到 Session。
- 每步结束后 poll steering（`consumeSteering`），有 steer 则继续循环。
- 内层循环结束后 blocking 等待 follow-up（`awaitFollowUp`），有新 Prompt/Steer 则重新进入内层循环。
...
`blades.NewAgent(name, opts...)` 创建默认 `llmAgent`。Agent 持有 model provider、tools、resolver、policy、hooks、compactor、prompt builder 等配置，全部通过 Option 注入。Memory 不在根 Agent 内置（由应用层通过 `prompt.Memory` section 注入）。**根包绑定默认 LLM Agent 执行语义**，loop/step/tool wave 逻辑全部内联在 `agent.go`；完全不同的 runtime 直接实现 `blades.Agent` 接口。
