...
`model/` 包定义 7 种内置 Part 类型（TextPart、FilePart、DataPart、ToolUsePart、ToolResultPart、ThinkingPart、CompactionSummaryPart）。5 策略分层压缩管线（ToolResultBudget → Snip → MicroCompact → AutoCompact → ReactiveCompact）按成本从低到高排列，token 降到预算内即短路。`prompt.Builder` 将 system prompt 分为静态可缓存前缀和动态后缀，配合 Provider 的 prompt cache 机制。
...
`ForkAgent` 通过共享静态 system prompt 前缀命中父 Agent 的 prompt cache。`BackgroundAgent` 支持 fire-and-forget 后台执行（Memory 提取、任务摘要）。`WorktreeConfig` 支持 git worktree 隔离。`QuerySource` 标记 fork 来源（user/sub_agent/compact/extract_memory/task_summary/skill），用于权限链和 Hook 的行为区分。
...
`retry.Policy` 感知 Provider 错误类型（Fatal/Retryable/RateLimit/Overloaded/AuthExpired），支持 529 模型过载自动降级和 401 认证刷新。`token.Counter` 接口支持 Provider 原生 / 字符估算 / 缓存包装三级降级。可观测性通过 Hook 系统集成 OpenTelemetry。`graph/` 保持独立子系统，通过 `flow/graph.go` 桥接为 `blades.Agent`。

### [迁移路径](design-migration.md)

核心接口迁移：`Agent.Run` 签名从 `Generator[*Message, error]` 改为 `(<-chan OutputEvent, error)`，`*Invocation` 去掉，`Middleware` 拆分为 `InputMiddleware` + `OutputMiddleware`。各包迁移：flow/ 适配新 Event 签名，contrib/ 实现 `model.Provider` 内部处理格式转换，skills/ 适配新 `tools.Tool` 接口，graph/ 保持独立通过 flow/ 桥接。
