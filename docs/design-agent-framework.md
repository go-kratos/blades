...
当前 Blades 已有 `Agent`、基于 `iter.Seq2` 的流式接口、`Session`、`middleware/`、`flow/`、`graph/`、`tools/`、`memory/` 和多 Provider 集成。本轮设计以当前 API 为基线，把这些能力重组为清晰分层的 AgentOS。
...
- 每个 step 边界非阻塞 drain active input：`agent_loop.go` 的 input queue helper 只分类 channel 事件；`Steer` 由同一文件中的 turn commit 路径写入当前 turn 下一步，`Prompt` 排队到下一 turn，`Abort` 结束当前 turn。
