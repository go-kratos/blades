...
tags: [agent, framework, architecture, context-management, memory, tools, permissions, hooks, session, streaming, streaming-optimization]
sub-docs:
...
│    ├── AgentOption         配置函数（WithModel/WithTools/...）   │
...
│    ├── Sub-Agent           后台/wortree 子 Agent                  │
│    └── Team                多 Agent 协调（Coordinator/Swarm）     │
├─────────────────────────────────────────────────────────────────┤
│  基础设施层                                                      │
...
| 原则 | 说明 | 参考来源 |
|------|------|---------|
| 极简根包 | 根包只放 Agent 接口 + Event 类型 + AgentOption + 组合原语（Sequential/Parallel/Loop）+ PromptBuilder，Message/Provider 下沉到 model/ 包 | 原创设计 |
...
| 一切皆 Agent | 预设 Agent（explore/plan/general/verify）是 agents/ 包中的构造函数，返回 Agent 接口。无 Role 工厂、无 ForkConfig 中间层。任何实现了 Agent 接口的 struct 就是 Agent | Claude Code built-in agents |
| 组合原语同包 | Sequential/Parallel/Loop 与 Agent 接口同包（类似 io.MultiReader），是 Agent 的基础组合能力 | Go 标准库 io 包先例 |
| 三层 Multi-Agent | L1 SubAgent（Spawn）→ L2 Coordinator → L3 Swarm/Team，每层基于前一层构建，用户按需选择复杂度。team/ 包统一管理多 Agent 协调 | Claude Code 三层 multi-agent 体系 |
| Fail-Closed 默认值 | 工具默认非并发安全、非只读；权限默认 ask；熔断器默认启用 | Claude Code fail-closed defaults |
| Prompt Cache 纪律 | 静态/动态分界、section 级缓存标记、子 agent 复用父级 prompt 字节 | Claude Code prompt cache discipline |
...
```
blades/                         根包：用户 API（Agent + Event + 组合原语 + PromptBuilder）
├── agent.go                    Agent 接口 + baseAgent 结构体 + New() 构造函数
├── event.go                    InputEvent / OutputEvent + 所有 Event 类型
├── options.go                  AgentOption + 所有 With* 函数
├── middleware.go               InputMiddleware / OutputMiddleware 类型
├── prompt.go                   PromptBuilder + Section + Breakpoint
├── errors.go                   公共错误
├── sequential.go               Sequential 顺序组合原语（实现 Agent）
├── parallel.go                 Parallel 并行组合原语（实现 Agent）
├── loop.go                     Loop 循环组合原语（实现 Agent）
├── spawn.go                    Spawn（parent, child, opts...）创建子 Agent
├── agent_tool.go               Tool(agent) → tools.Tool 适配器
│
├── agents/                     预设 Agent 实现（可替换的参考实现）
│   ├── explore.go              agents.Explore() — 快速只读代码搜索
│   ├── plan.go                 agents.Plan() — 架构设计与实现规划
│   ├── general.go              agents.General() — 全能力通用 Agent
│   └── verify.go               agents.Verify() — 对抗性验证
│
...
│   ├── filter.go               tools.ToolFilter + ReadOnlyTools/AllowOnly/Disallow/And/Or
│   └── exit.go                 tools.ExitTool
│
├── memory/                     Memory 系统（依赖反转，不导入 blades/）
│   ├── store.go                memory.Store 接口
│   ├── loader.go               memory.Loader
│   ├── entry.go                memory.Entry / memory.Type
│   ├── extractor.go            memory.Extractor（自动提取，ForkFunc 由上层注入）
...
│   └── bridge.go               PermissionBridge（InProcess / Mailbox）
│
├── graph/                      DAG 执行器（可选子系统）
...
| 变化 | 原因 |
|------|------|
| 根包精简为 Agent + Event + AgentOption + 组合原语 + PromptBuilder | Message/Provider 下沉到 model/ 包，根包只保留用户 API 和基础组合能力 |
| 新增 `model/` | 合并 Message + Provider 为纯类型包，是依赖图的叶子节点 |
| 新增 `agents/` | 预设 Agent 实现（Explore/Plan/General/Verify）作为可替换的参考实现，不放在根包 |
| 新增 `team/` | 多 Agent 协调（Coordinator/Swarm/Team）独立为包，与基础 Agent 概念分离 |
...
| `prompt/` 包去掉 | 仅 PromptBuilder + Section 两个类型，放根包。internal/loop/ 不操作 prompt（只接收最终 string），无循环依赖 |
| `flow/` 去掉，组合原语迁入根包 | Sequential/Parallel/Loop 与 Agent 接口同包（类似 io.MultiReader/io.PipeReader），是 Agent 的基础组合能力。RoutingAgent 被 AgentTool + subagent_type 路由替代，DeepAgent 的 todo/task 能力内化为框架内置工具。Coordinator/Swarm 模式在 team/ 包中实现 |
| `graph/` 降级为可选子系统 | DAG 执行器操作 State map，不是 Agent 接口，更适合数据管道场景，不在核心依赖路径上 |
| 去掉 `agent/` 包 | 原 agent/ 包内容拆分：Role 去掉（改为 Agent 接口 + AgentOption 模式），组合模式迁入根包，Coordinator/Team 迁入 team/，ToolFilter 迁入 tools/，预设 Agent 迁入 agents/ |
...
model/（叶子包：Message + Provider + Request/Response，不依赖任何 blades 子包）
  ...
  ├── permission/（独立）
  ├── retry/（独立）
  ├── memory/（依赖 model/，不依赖 blades/：ForkFunc 由上层注入）
  ├── settings/（独立）
  ↑
  ├── skills/（依赖 tools/）
  ↑
  ├── blades/（根包：Agent 接口 + 组合原语 + PromptBuilder。依赖 model/, session/, compact/, tools/, hook/, permission/, retry/, settings/）
  │   └── Agent Loop 内部实现 ContextBuilder（含消息过滤）和 streamAndRecord
  │   └── agents/（依赖 blades/ 根包 + tools/ + model/：预设 Agent 实现）
  ↑
  ├── team/（依赖 blades/ 根包 + tools/ + permission/ + model/ + session/：Coordinator + Swarm）
  ├── middleware/（依赖 blades/ 根包）
  ├── recipe/（依赖 blades/, tools/, model/）
  ├── evaluator/（依赖 blades/）
...
根包依赖 model/ 是向下依赖。compact/ 通过 `Summarizer` 函数注入避免对根包的循环依赖。memory/ 通过注入 `ForkFunc` 避免对根包的循环依赖。

### 各包的核心导出类型

| 包 | 核心类型 | 读作 |
|---|---------|------|
| `blades` | `Agent`, `InputEvent`, `OutputEvent`, `AgentOption`, `PromptBuilder`, `Sequential`, `Parallel`, `Loop` | `blades.Agent` |
| `agents` | `Explore`, `Plan`, `General`, `Verify`（均返回 `blades.Agent`） | `agents.Explore()` |
| `model` | `Message`, `Provider`, `Request`, `Response`, `Part`, `TokenUsage`, `Counter` | `model.Provider` |
| `session` | `Session`, `Store`, `Writer`, `Entry`, `Snapshot`, `Tree`, `Header`, `State` | `session.Session` |
| `tools` | `Tool`, `ToolFilter`, `ConcurrentTool`, `ReadOnlyTool`, `Handler`, `Resolver` | `tools.Tool` |
...
| `team` | `Coordinator`, `Team`, `Mailbox`, `TaskList`, `PermissionBridge` | `team.Coordinator` |
...
`model/` 包定义 7 种内置 Part 类型（TextPart、FilePart、DataPart、ToolUsePart、ToolResultPart、ThinkingPart、CompactionSummaryPart）。6 策略分层压缩管线（ToolResultBudget → Snip → MicroCompact → AutoCompact → ReactiveCompact → SessionMemoryCompact → PostCompactRestore）按成本从低到高排列，token 降到预算内即短路。`blades.PromptBuilder` 将 system prompt 分为静态可缓存前缀和动态后缀，配合 Provider 的 prompt cache 机制。

### [工具系统](design-tool-system.md)

核心 `Tool` 接口保持精简（4 方法），扩展能力通过可选接口（`ConcurrentTool`、`ReadOnlyTool`、`DestructiveTool`、`PromptContributor`、`BudgetedTool`）实现。`tools.ToolFilter` 函数类型 + `ReadOnlyTools`/`AllowOnly`/`Disallow`/`And`/`Or` 组合器，用于过滤工具集。`StreamingToolExecutor` 在模型流式输出时提前启动并发安全工具。`partitionToolCalls` 自动按并发模式分组。工具执行完整生命周期：参数校验 → BeforeToolHook → 权限检查 → Handle → AfterToolHook → ToolResultBudget → 事件发射。
...
`blades.Spawn(parent, child, opts...)` 从任意 Agent 创建子 Agent，支持共享静态 system prompt 前缀命中父 Agent 的 prompt cache。`BackgroundAgent` 支持 fire-and-forget 后台执行（Memory 提取、任务摘要）。`WorktreeConfig` 支持 git worktree 隔离。`QuerySource` 标记 fork 来源（user/sub_agent/compact/extract_memory/task_summary/skill），用于权限链和 Hook 的行为区分。预设 Agent 实现在 `agents/` 包中，框架内置 4 种（explore/plan/general/verify），每个都是返回 `blades.Agent` 的构造函数，用户可自定义。组合模式（Sequential/Parallel/Loop）在根包中，与 Agent 接口同包。
...
核心接口迁移：`Agent.Run` 签名从 `Generator[*Message, error]` 改为 `(<-chan OutputEvent, error)`，`*Invocation` 去掉，`Middleware` 拆分为 `InputMiddleware` + `OutputMiddleware`。各包迁移：flow/ 中 Sequential/Parallel/Loop 迁入根包并适配新 Event 签名，RoutingAgent 和 DeepAgent 不再保留，contrib/ 实现 `model.Provider` 内部处理格式转换，skills/ 适配新 `tools.Tool` 接口，graph/ 保持为可选独立子系统。新增 Coordinator 模式和 Swarm/Team 模式（无需迁移现有代码）。

### [多 Agent 协调](design-team.md)

`team/` 包管理多 Agent 协调。三层 Multi-Agent 体系：L1 SubAgent（`blades.Spawn`）→ L2 Coordinator（主线程变调度器，worker 结果以 notification 回流）→ L3 Swarm/Team（显式团队实体，共享任务列表，mailbox 通信，权限桥接）。`team.Coordinator` 通过 system prompt 重写主 Agent 角色为调度器，`team.Team` 提供持久化团队实体，`team.Mailbox` 基于文件锁实现 Agent 间通信，`team.TaskList` 提供共享任务队列。
...
- [ ] 实现 `blades.PromptBuilder`（静态/动态分段，放根包）
...
- [ ] 集成 ModeManager 到 Agent Loop（ContextBuilder + PromptBuilder）

### 阶段 7：子 Agent 系统 + 组合原语 + 预设 Agent

- [ ] 实现 `blades.Spawn(parent, child, opts...)`（共享缓存前缀 + QuerySource 行为区分）
- [ ] 实现 `BackgroundAgent`（fire-and-forget + Drain）
- [ ] 实现 `CreateWorktreeAgent`（git worktree 隔离）
- [ ] 实现 `QuerySource` 行为区分
- [ ] 实现 `tools.ToolFilter` + 预定义过滤器 ReadOnlyTools/AllowOnly/Disallow/And/Or（tools/filter.go）
- [ ] 实现 Sequential/Parallel/Loop 到根包（与 Agent 接口同包）
- [ ] 实现 `blades.Tool(agent)` — Agent → Tool 适配器（agent_tool.go）
- [ ] 实现 `agents/` 包：4 种预设构造函数（Explore/Plan/General/Verify），均返回 `blades.Agent`
- [ ] 去掉 flow/ 包（RoutingAgent、DeepAgent 不再保留）
- [ ] graph/ 降级为可选子系统

### 阶段 7b：多 Agent 协调（team/ 包）

- [ ] 实现 `team.CoordinatorConfig` + `team.NewCoordinator`
- [ ] 实现 `CoordinatorSystemPrompt` 生成（角色定义 + worker 列表 + 工作流分相 + 约束规则）
- [ ] 实现 `TaskNotificationEvent` 回流机制
- [ ] 实现 `team.Team` + `team.TeamConfig` + `team.TeamStore`
- [ ] 实现 `team.NewFileTeamStore`（JSON 文件持久化）
- [ ] 实现 `team.Mailbox`（文件锁 + 并发安全）
- [ ] 实现 `team.TaskList`（共享任务平面）
- [ ] 实现 `team.PermissionBridge`（InProcess + Mailbox 双轨）
...
- [ ] 迁移 Sequential/Parallel/Loop 到根包并适配新 Event 接口
- [ ] 迁移 `contrib/` Provider 实现（实现 model.Provider，内部处理格式转换）
- [ ] 迁移 `skills/` 到新 Tool 接口
- [ ] `agents/` 预设 Agent 和 `team/` 多 Agent 协调为新增模块，无需迁移
...
| 现有代码迁移工作量 | 高 | 阶段 10 专门处理迁移，contrib/skills 逐包迁移 |
...
| 去掉 RoutingAgent 后路由能力下降 | 低 | blades.Tool(agent) + subagent_type 提供更灵活的路由，由 LLM 自行决策 |
| 去掉 DeepAgent 后复杂任务编排能力下降 | 中 | TaskCreate/TaskUpdate 内置工具 + 根包组合原语覆盖相同场景 |
| Coordinator 模式 worker 失控 | 中 | MaxWorkers 限制 + worker 不能 spawn worker 约束 + TaskStop 统一 kill switch |
| team/ mailbox 文件锁竞争 | 中 | 锁超时 + 重试机制，单 inbox 文件写入频率低 |
| team/ 权限桥接延迟 | 中 | InProcessBridge 零延迟（同进程），MailboxBridge 有轮询延迟但可配置间隔 |
| Teammate 拓扑失控 | 高 | 硬约束：teammate 不能 spawn teammate，in-process teammate 不能启动 background agent |
| 根包文件数膨胀 | 低 | ~12 文件（Agent 接口 + Event + Option + 组合原语 + PromptBuilder），规模与 io/database/sql 相当 |
...
- Pause：Agent Loop 停止从 Provider stream 读取，但不断开连接。如果 Agent 正在 StopHooks 阶段，Pause 被忽略——StopHooks 必须完成（Memory 提取、session memory 更新等后台工作不可中断）
- Resume：恢复读取
