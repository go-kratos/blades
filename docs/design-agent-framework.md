...
`retry.Policy` 感知 Provider 错误类型（Fatal/Retryable/RateLimit/Overloaded/AuthExpired），支持 529 模型过载自动降级和 401 认证刷新。`model.Counter` 接口支持 Provider 原生 / 字符估算 / 缓存包装三级降级。可观测性通过 Hook 系统集成 OpenTelemetry。`graph/` 保持独立子系统，通过 `flow/graph.go` 桥接为 `blades.Agent`。
