...
- `ToolResultBudget` 只对单条消息内的 `ToolPart.Response` 做长度截断，不跨消息累计状态。
...
- **Step 间（Loop 透传 hint）**：若 provider 实际调用仍返回 context-too-long 错误，Loop 通过 `compact.WithHint(ctx, HintShrink)` 透传 hint 进入**同一 step 的第二次** `RequestBuilder` 调用。Compactor 在 hint 模式下应采取更激进的策略（例如降低 `KeepRecent`、启用更紧的 `ToolResultBudget`），并必须返回**严格单调下降**的视图（token 数严格小于上一次返回的视图）。最大重试次数默认 1 次；若返回视图未严格下降或仍超预算，Loop **fail-fast** 抛出 `event.Error` 并终止 turn。
...
- **调用顺序（在 RequestBuilder 内部）**：
  ...
  ```
  Compactor 不读 `system`，prompt builder 也不读 `view`；二者在 RequestBuilder 处汇合，但不互相依赖。
...
9. **Memory 与 Compact 解耦**：Memory 走 system 段、Compact 走 messages 段；两者在 RequestBuilder 汇合但互不感知；预算由应用层三段分摊，不互相兜底。

## 与相邻设计的接口面

- [`design-event-agent-loop.md`](design-event-agent-loop.md)：触发条件、RequestBuilder pipeline、HintShrink 重试与 Compactor 内部迭代的两层关系。
