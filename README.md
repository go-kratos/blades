...
2. Register with Agent via `WithModel()`
...
- **InMemory**: Built-in short-term memory
- **Custom**: Implement `Memory` interface for persistence
...
Skills can be loaded from a directory or `embed.FS`:
...
For skill package format, see [Agent Skill specification](https://agentskills.io/specification).
...
3. Error handling in Handle method
