...
func TestRegisterFactoryAndCreate(t *testing.T) {
	r := NewStaticModelRegistry()
	r.RegisterFactory("zhipu", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: "zhipu/" + model}, nil
	})

	m, err := r.Create("zhipu", "glm-5")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if m.Name() != "zhipu/glm-5" {
		t.Fatalf("expected %q, got %q", "zhipu/glm-5", m.Name())
	}
}

func TestCreateUnknownProvider(t *testing.T) {
	r := NewStaticModelRegistry()
	_, err := r.Create("nonexistent", "model")
	if err == nil {
		t.Fatal("expected error for unknown provider")
	}
	if !strings.Contains(err.Error(), "nonexistent") {
		t.Fatalf("error should mention provider: %v", err)
	}
}

func TestRegisterFactoryOverwrite(t *testing.T) {
	r := NewStaticModelRegistry()
	r.RegisterFactory("p", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: "v1/" + model}, nil
	})
	r.RegisterFactory("p", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: "v2/" + model}, nil
	})
	m, err := r.Create("p", "x")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if m.Name() != "v2/x" {
		t.Fatalf("expected overwritten factory, got %q", m.Name())
	}
}

func TestBuildWithProvider(t *testing.T) {
	registry := NewStaticModelRegistry()
	registry.RegisterFactory("zhipu", func(model string) (blades.ModelProvider, error) {
		return &captureRequestModel{name: model}, nil
	})
...
func TestBuildWithProviderRunsCorrectly(t *testing.T) {
	model := &captureRequestModel{name: "glm-5", response: "hello from glm"}
	registry := NewStaticModelRegistry()
	registry.RegisterFactory("zhipu", func(name string) (blades.ModelProvider, error) {
		if name == "glm-5" {
			return model, nil
		}
	...
	spec := &RecipeSpec{
		...
		Provider:    "zhipu",
		Model:       "glm-5",
		Instruction: "Review {{.language}} code",
		Parameters:  []ParameterSpec{{Name: "language", Type: ParameterString, Required: ParameterRequired}},
	}
	agent, err := Build(spec, WithModelRegistry(registry), WithParams(map[string]any{"language": "go"}))
	if err != nil {
	...
	msg, err := blades.NewRunner(agent).Run(context.Background(), blades.UserMessage("check"))
	if err != nil {
		t.Fatalf("run failed: %v", err)
	}
	if msg.Text() != "hello from glm" {
		t.Fatalf("unexpected output: %q", msg.Text())
	}
	if !strings.Contains(model.instruction, "Review go code") {
		t.Fatalf("instruction not rendered: %q", model.instruction)
...
func TestBuildWithProviderUnknown(t *testing.T) {
	registry := NewStaticModelRegistry()
	spec := &RecipeSpec{
		Version:     "1.0",
		Name:        "test",
	...
	_, err := Build(spec, WithModelRegistry(registry))
	if err == nil {
		t.Fatal("expected error for unknown provider")
	}
}

func TestBuildSubRecipeInheritsProvider(t *testing.T) {
	registry := NewStaticModelRegistry()
	registry.RegisterFactory("zhipu", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: model}, nil
	})
	...
	spec := &RecipeSpec{
		...
		Execution:   ExecutionSequential,
		SubRecipes: []SubRecipeSpec{
			{Name: "step1", Instruction: "do a"},
			{Name: "step2", Instruction: "do b"},
		},
	}
	agent, err := Build(spec, WithModelRegistry(registry))
	if err != nil {
...
func TestBuildSubRecipeOverridesProvider(t *testing.T) {
	registry := NewStaticModelRegistry()
	registry.RegisterFactory("zhipu", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: "zhipu/" + model}, nil
	})
	registry.RegisterFactory("deepseek", func(model string) (blades.ModelProvider, error) {
		return &mockModel{name: "deepseek/" + model}, nil
	})
	...
	spec := &RecipeSpec{
		...
		Execution:   ExecutionSequential,
		SubRecipes: []SubRecipeSpec{
			{Name: "step1", Instruction: "do a"},                                           // inherits zhipu + glm-5
			{Name: "step2", Provider: "deepseek", Model: "deepseek-r2", Instruction: "do b"}, // overrides
		},
	}
	agent, err := Build(spec, WithModelRegistry(registry))
	if err != nil {
...
func TestBuildToolModeWithProvider(t *testing.T) {
	model := &captureRequestModel{name: "glm-5"}
	registry := NewStaticModelRegistry()
	registry.RegisterFactory("zhipu", func(name string) (blades.ModelProvider, error) {
		if name == "glm-5" {
			return model, nil
...
func TestBuildWithProviderFromYAML(t *testing.T) {
	yaml := `
version: "1.0"
name: zhipu-test
provider: zhipu
...
`
	...
	if spec.Provider != "zhipu" {
		t.Fatalf("expected provider %q, got %q", "zhipu", spec.Provider)
	}
	if spec.Model != "glm-5" {
		t.Fatalf("expected model %q, got %q", "glm-5", spec.Model)
	}
	...
	if agent.Name() != "zhipu-test" {
		t.Fatalf("unexpected name: %q", agent.Name())
	}
}

func TestBuildWithoutProviderStillWorks(t *testing.T) {
	// Backward compatibility: no provider field, exact name match
	registry := NewStaticModelRegistry()
	registry.Register("gpt-4o", &mockModel{name: "gpt-4o"})
